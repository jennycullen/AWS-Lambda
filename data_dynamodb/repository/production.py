...
class DynamoProductionRepository(ProductionRepository, SQSManager, SnsNotifier):
    ...
    def get_production_by_production_date_range(self, supplier_id, min_production_date, max_production_date=None):
        ...
        if max_production_date:
            query = {
                'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('production_date').between(min_production_date,
                                                                                                       max_production_date),
                'FilterExpression':
                    Attr('latest').eq(True) & Attr('active').eq(True),
                'IndexName': 'by_supplier_id_and_production_date'
            ...
            query = {
                'KeyConditionExpression': Key('supplier_id').eq(supplier_id) & Key('production_date').gt(min_production_date),
                'FilterExpression':
                    Attr('latest').eq(True) & Attr('active').eq(True),
                'IndexName': 'by_supplier_id_and_production_date'
            }
